...
export function getDumpDirectory(): string {
  ...
  const videoThumbsDir = path.join(dumpDir, 'thumbnails_video');
  const pdfThumbsDir = path.join(dumpDir, 'thumbnails_pdf');
  try {
    for (const d of [dumpDir, thumbsDir, videoThumbsDir, pdfThumbsDir]) {
      if (!fs.existsSync(d)) {
 ...
 */
export function getZipCachePath(type: 'video' | 'pdf' | 'legacy' = 'legacy'): string {
  const dumpDir = getDumpDirectory();
  if (type === 'video') return path.join(dumpDir, 'video_thumbnails_cache.zip');
  if (type === 'pdf') return path.join(dumpDir, 'pdf_thumbnails_cache.zip');
  return path.join(dumpDir, 'thumbnails_cache.zip');
}
...
export function setArchivePassword(pw: string) { archivePassword = pw; }
export function getArchivePassword(): string { return archivePassword; }
export function setTagFilePassword(pw: string) { tagFilePassword = pw; }
export function getTagFilePassword(): string { return tagFilePassword; }
...
function decryptBuffer(data: Buffer, password: string): Buffer {
  ...
  try {
    ...
    console.warn('[ZipCache] Decryption failed, returning raw data');
    return data;
  }
}
    ...
    if (fs.existsSync(zipPath)) {
      try {
        ...
        for (const entry of zipEntries) {
          if (entry.entryName.endsWith('.webp') || entry.entryName.endsWith('.jpg') || entry.entryName.endsWith('.jpeg')) {
            const key = entry.entryName.replace(/\.(webp|jpe?g)$/i, '');
            const buffer = entry.getData();
            if (buffer && buffer.length > 0) {
              const ext = entry.entryName.endsWith('.webp') ? 'webp' : 'jpeg';
...
export function saveThumbnailToZip(
  ...
  // Save individual image file to dump/thumbnails/[key].webp immediately
  try {
    const thumbsDir = getThumbsDirectory();
    const ext = dataUrl.includes('image/webp') ? 'webp' : 'jpg';
    const filePath = path.join(thumbsDir, `${key}.${ext}`);
...
export function saveBatchThumbnailsToZip(items: CachedItem[]): void {
  ...
  for (const item of items) {
    ...
    try {
      const thumbsDir = getThumbsDirectory();
      const ext = item.dataUrl.includes('image/webp') ? 'webp' : 'jpg';
      const filePath = path.join(thumbsDir, `${key}.${ext}`);
    ...
    for (const [key, dataUrl] of inMemoryThumbnails.entries()) {
      if (/^[a-f0-9]{32}$/i.test(key) && dataUrl && dataUrl.startsWith('data:image')) {
        ...
        if (imageBuffer.length > 0) {
          const ext = dataUrl.includes('image/webp') ? 'webp' : 'jpg';
          zip.addFile(`${key}.${ext}`, imageBuffer);
          addedCount++;
        }
      }
    }
...
/**
 * Flush thumbnails into separate password-encrypted archives for video and PDF thumbnails.
 */
export async function flushEncryptedArchives(): Promise<void> {
  const dumpDir = getDumpDirectory();
  const manifestObj: Record<string, ThumbnailMetadata> = {};
  for (const [k, v] of inMemoryManifest.entries()) {
    manifestObj[k] = v;
  ...
  // Separate video vs PDF thumbnails based on manifest metadata
  const videoEntries: [string, string][] = [];
  const pdfEntries: [string, string][] = [];

  for (const [key, dataUrl] of inMemoryThumbnails.entries()) {
    if (!/^[a-f0-9]{32}$/i.test(key) || !dataUrl || !dataUrl.startsWith('data:image')) continue;
    const meta = inMemoryManifest.get(key);
    if (meta?.isPdf) {
      pdfEntries.push([key, dataUrl]);
    } else {
      videoEntries.push([key, dataUrl]);
    }
  }

  // Build and write video archive
  if (videoEntries.length > 0) {
    try {
      const zip = new AdmZip();
      for (const [key, dataUrl] of videoEntries) {
        const base64Data = dataUrl.replace(/^data:image\/\w+;base64,/, '');
        const ext = dataUrl.includes('image/webp') ? 'webp' : 'jpg';
        zip.addFile(`${key}.${ext}`, Buffer.from(base64Data, 'base64'));
      }
      zip.addFile('manifest.json', Buffer.from(JSON.stringify(
        Object.fromEntries(Object.entries(manifestObj).filter(([_, v]) => !v.isPdf)), null, 2
      ), 'utf8'));

      const zipBuffer = zip.toBuffer();
      const finalBuffer = archivePassword ? encryptBuffer(zipBuffer, archivePassword) : zipBuffer;
      await fs.promises.writeFile(path.join(dumpDir, 'video_thumbnails_cache.zip'), finalBuffer);
      console.log(`[ZipCache] Encrypted video archive: ${videoEntries.length} thumbnails`);
    } catch (err) {
      console.error('[ZipCache] Video archive error:', err);
    }
  }

  // Build and write PDF archive
  if (pdfEntries.length > 0) {
    try {
      const zip = new AdmZip();
      for (const [key, dataUrl] of pdfEntries) {
        const base64Data = dataUrl.replace(/^data:image\/\w+;base64,/, '');
        const ext = dataUrl.includes('image/webp') ? 'webp' : 'jpg';
        zip.addFile(`${key}.${ext}`, Buffer.from(base64Data, 'base64'));
      }
      ...
      const zipBuffer = zip.toBuffer();
      const finalBuffer = archivePassword ? encryptBuffer(zipBuffer, archivePassword) : zipBuffer;
      await fs.promises.writeFile(path.join(dumpDir, 'pdf_thumbnails_cache.zip'), finalBuffer);
      console.log(`[ZipCache] Encrypted PDF archive: ${pdfEntries.length} thumbnails`);
    } catch (err) {
      console.error('[ZipCache] PDF archive error:', err);
    }
  }
}

/**
 * Save global tag array as password-protected text file in dump folder.
 */
export async function saveTagsFile(tags: string[]): Promise<void> {
  const dumpDir = getDumpDirectory();
  const tagContent = JSON.stringify(tags, null, 2);
  const buffer = Buffer.from(tagContent, 'utf8');
  const finalBuffer = tagFilePassword ? encryptBuffer(buffer, tagFilePassword) : buffer;
  const ext = tagFilePassword ? '.dat' : '.txt';
  await fs.promises.writeFile(path.join(dumpDir, `tags${ext}`), finalBuffer);
  console.log(`[ZipCache] Saved ${tags.length} tags to dump folder`);
}

/**
 * Load tags from password-protected file in dump folder.
 */
export async function loadTagsFile(): Promise<string[]> {
  const dumpDir = getDumpDirectory();
  const encPath = path.join(dumpDir, 'tags.dat');
  const txtPath = path.join(dumpDir, 'tags.txt');
  ...
  try {
    ...
    const buffer = filePath.endsWith('.dat') && tagFilePassword
      ? decryptBuffer(raw, tagFilePassword)
      : raw;
    return JSON.parse(buffer.toString('utf8'));
  } catch (err) {
    console.warn('[ZipCache] Failed to load tags:', err);
    return [];
  }
}

/**
 * Load passwords from persistent settings file.
 */
export async function loadPasswords(): Promise<{ archivePassword: string; tagFilePassword: string }> {
  const dumpDir = getDumpDirectory();
  const settingsPath = path.join(dumpDir, '.settings.json');
  try {
    if (fs.existsSync(settingsPath)) {
      const data = JSON.parse(await fs.promises.readFile(settingsPath, 'utf8'));
      if (data.archivePassword) { archivePassword = data.archivePassword; setArchivePassword(data.archivePassword); }
      if (data.tagFilePassword) { tagFilePassword = data.tagFilePassword; setTagFilePassword(data.tagFilePassword); }
      return { archivePassword: data.archivePassword || '', tagFilePassword: data.tagFilePassword || '' };
    }
  } catch (_) {}
  return { archivePassword: '', tagFilePassword: '' };
}

/**
 * Save passwords to persistent settings file.
 */
export async function savePasswords(archivePw: string, tagPw: string): Promise<void> {
  archivePassword = archivePw;
  tagFilePassword = tagPw;
  setArchivePassword(archivePw);
  setTagFilePassword(tagPw);
  const dumpDir = getDumpDirectory();
  const settingsPath = path.join(dumpDir, '.settings.json');
  await fs.promises.writeFile(settingsPath, JSON.stringify({ archivePassword: archivePw, tagFilePassword: tagPw }, null, 2), 'utf8');
}
