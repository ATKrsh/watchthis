...
function setupIPC() {
  ...
  // ── Encrypted Archive & Tag IPC Channels ──
  ipcMain.handle('cache:flushEncryptedArchives', async () => {
    await flushEncryptedArchives();
    return true;
  });

  ipcMain.handle('tags:save', async (_event, tags: string[]) => {
    await saveTagsFile(tags);
    return true;
  });
  ...
  ipcMain.handle('settings:getPasswords', async () => {
    return loadPasswords();
  });

  ipcMain.handle('settings:setPasswords', async (_event, archivePw: string, tagPw: string) => {
    await savePasswords(archivePw, tagPw);
    return true;
  });
