        ...
        const tryCaptureFrame = (): boolean => {
          try {
            ...
            if (vw > 0 && vh > 0) {
              ...
              const targetHeight = Math.max(120, Math.round((targetWidth / vw) * vh)) || 495;
    ...
    return new Promise<string>((resolve) => {
      const executeTask = async () => {
        ...
        try {
          ...
          // Scale to target 630px height
          const scale = Math.min(2.0, maxHeight / unscaledViewport.height);
