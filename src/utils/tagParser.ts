...
/**
 * Builds the global deduplicated tag array across all file names:
 * 1. Takes all filename strings
 * 2. Cleans entities, brackets, domains, and extensions
 * 3. Removes common filler words (is, as, the, to, in, on, me, you, etc.)
 * 4. Extracts strictly valid words, deduplicating them into a global array
 */
export function buildGlobalTagArray(filenames: string[]): string[] {
  const globalSet = new Set<string>();
  const globalArray: string[] = [];

  for (const filename of filenames) {
    if (!filename) continue;
    const keywords = extractCleanKeywords(filename);
    for (const kw of keywords) {
      const lower = kw.toLowerCase();
      if (!globalSet.has(lower)) {
        globalSet.add(lower);
        globalArray.push(kw);
      }
    }
  ...
  // Sort alphabetically
  return globalArray.sort((a, b) => a.localeCompare(b));
}
