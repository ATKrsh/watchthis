...
import { parseVideoMetadata, extractCleanKeywords, buildGlobalTagArray } from '../utils/tagParser';
import { initZipCacheLoader, getMemoryThumbnail, setMemoryThumbnail, getZipCachedMetadata, setStoredThumbnail, clearAllStoredThumbnails } from '../utils/thumbnailDb';
...
export interface IndexingOperationDetail {
  total: number;
  current: number;
  stageNum: number;
  totalStages: number;
...
export interface IndexingStageState {
  stage: 'idle' | 'indexing' | 'caching' | 'archiving' | 'complete';
  scanCount: number;
  totalFound: number;
  cachedCount: number;
  totalToCache: number;
  ...
  currentItem?: string;
  video: IndexingOperationDetail;
  pdf: IndexingOperationDetail;
  tags: IndexingOperationDetail;
  archive: {
...
interface LibraryContextType {
  ...
  indexingStage: IndexingStageState;
}
...
export const LibraryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [indexingStage, setIndexingStage] = useState<IndexingStageState>({
    stage: 'idle',
    scanCount: 0,
    totalFound: 0,
    cachedCount: 0,
  ...
  // Background Thumbnail, Tag Extraction & Archive Pipeline (Multi-Stage Progress)
  useEffect(() => {
    if (!showThumbnails || videos.length === 0) return;
    let isCancelled = false;

    // 1. Tag extraction & save check
    const allFilenames = videos.map(v => v.name);
    const globalTags = buildGlobalTagArray(allFilenames);
    
    // Save tags to dump folder if Electron is present
    if (window.electronAPI?.saveTags && globalTags.length > 0) {
      window.electronAPI.saveTags(globalTags).catch(() => {});
    }
    ...
    const videoUncached = unCached.filter(v => !v.isPdf && v.extension?.toLowerCase() !== '.pdf');
    const pdfUncached = unCached.filter(v => v.isPdf || v.extension?.toLowerCase() === '.pdf');

    if (unCached.length === 0) {
      setIndexingStage(prev => (prev.stage === 'caching' ? {
        ...prev,
        stage: 'idle',
        video: { ...prev.video, active: false, current: prev.video.total, stageNum: 6, stageLabel: 'Archived' },
        pdf: { ...prev.pdf, active: false, current: prev.pdf.total, stageNum: 6, stageLabel: 'Archived' },
        tags: { ...prev.tags, active: false, current: prev.tags.total, stageNum: 8, stageLabel: 'Protected', extractedCount: globalTags.length },
      } : prev));
      return;
    }
    ...
    setIndexingStage(prev => ({
      ...
      video: {
        total: videoUncached.length,
        current: 0,
        stageNum: 1,
        totalStages: 6,
        stageLabel: videoUncached.length > 0 ? '1/6: Extracting frame' : 'Complete',
        active: videoUncached.length > 0,
      },
      pdf: {
        total: pdfUncached.length,
        current: 0,
        stageNum: 1,
        totalStages: 6,
        stageLabel: pdfUncached.length > 0 ? '1/6: Extracting page' : 'Complete',
        active: pdfUncached.length > 0,
      },
      tags: {
        total: videos.length,
        ...
        stageNum: 8,
        totalStages: 8,
        stageLabel: '8/8: Tags Encrypted',
        active: false,
        extractedCount: globalTags.length,
      },
      archive: {
        videoArchived: false,
        pdfArchived: false,
        ...
        try {
          ...
          if (!thumbUrl) {
            ...
            thumbUrl = await Promise.race([
              isPdf
                ? generatePdfThumbnail(source, item.id, 630)
          ...
          completedInRun++;
          if (isPdf) pdfDone++; else videoDone++;
          ...
          if (thumbUrl && !isCancelled) {
            ...
            // Immediately apply thumbnail to card state for ASAP rendering
            setVideos(prev =>
              prev.map(v => (v.id === item.id || v.path === item.path ? { ...v, thumbnail: thumbUrl } : v))
            );
          } else if (!isCancelled) {
            // Mark failed / unsupported files in memory cache so queue never loops on them
          ...
          const now = Date.now();
          if (now - lastProgressUpdate > 120 && !isCancelled) {
            ...
            setIndexingStage(prev => ({
              ...
              video: {
                ...prev.video,
                current: Math.min(prev.video.total, videoDone),
                stageNum: isPdf ? prev.video.stageNum : 4,
                stageLabel: isPdf ? prev.video.stageLabel : '4/6: Saving WebP to dump',
                active: videoUncached.length > videoDone,
              },
              pdf: {
                ...prev.pdf,
                current: Math.min(prev.pdf.total, pdfDone),
                stageNum: isPdf ? 4 : prev.pdf.stageNum,
                stageLabel: isPdf ? '4/6: Saving WebP to dump' : prev.pdf.stageLabel,
                active: pdfUncached.length > pdfDone,
              },
            }));
          }
        } catch (_) {
    ...
    Promise.all(pool).then(async () => {
      if (!isCancelled) {
        // Flush encrypted archives for Video and PDF thumbnails
        if (window.electronAPI?.flushEncryptedArchives) {
          try {
            await window.electronAPI.flushEncryptedArchives();
          } catch (_) {}
        } else if (window.electronAPI?.flushZipCache) {
        ...
        setIndexingStage(prev => ({
          ...prev,
          stage: 'idle',
          cachedCount: prev.totalToCache,
          video: {
            ...prev.video,
            current: prev.video.total,
            stageNum: 6,
            stageLabel: '6/6: Archived',
            active: false,
          },
          pdf: {
            ...prev.pdf,
            current: prev.pdf.total,
            stageNum: 6,
            stageLabel: '6/6: Archived',
            active: false,
          },
  ...
  const reindexAllFiles = async () => {
    ...
    try {
      // Clear existing thumbnail cache to force complete fresh rebuild
      await clearAllStoredThumbnails();
