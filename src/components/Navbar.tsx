  ...
  return (
    ...
    <header className="h-16 px-3 border-b border-white/[0.12] bg-surface/95 backdrop-blur-2xl flex items-center justify-between gap-3 z-40 select-none font-sans app-drag-region overflow-hidden flex-shrink-0">
      ...
      <div className="flex items-center gap-1.5 app-no-drag flex-shrink-0 overflow-x-auto no-scrollbar py-1">
        ...
        {/* Group 2: Refresh, Reindex, Library Data, Settings */}
        <div className="flex items-center bg-surface-elevated/80 p-0.5 border border-white/[0.12] shadow-sm">
          {/* Button 1: Refresh (Quick file link check) */}
          <button
            onClick={async () => {
              setIsRefreshing(true);
              try { await refreshLibrary(); } finally { setIsRefreshing(false); }
            }}
            disabled={isBusy || isRefreshing}
            title="Refresh — Quick re-check & re-link file paths (no rebuild)"
            className={`flex items-center gap-1.5 px-2 h-7 text-xs font-mono transition-all ${
              isRefreshing
                ? 'bg-accent-neon/20 text-accent-neon font-semibold animate-pulse'
                : 'text-slate-300 hover:text-white hover:bg-white/[0.06]'
            }`}
          >
            <RotateCcw className={`w-3.5 h-3.5 ${isRefreshing ? 'animate-spin text-accent-neon' : ''}`} />
            <span className="text-[10px] font-mono font-medium hidden xl:inline">
              {isRefreshing ? 'Refreshing...' : 'Refresh'}
            </span>
          </button>

          {/* Button 2: Force Reindex All Files with Live ETA */}
          <button
            onClick={() => reindexAllFiles()}
            disabled={isBusy}
            title={`Force Re-index All Files — Rebuild thumbnails, tags & archives ${indexingEtaFormatted ? `(${indexingEtaFormatted})` : ''}`}
          ...
          {/* Button 3: Library Data (Audit & Diagnostics) */}
          <button
          ...
          >
            ...
            <span className="text-[10px] font-mono font-medium hidden xl:inline">Data</span>
          </button>
          ...
          <button
            ...
            className="flex items-center justify-center w-7 h-7 text-slate-400 hover:text-accent-neon hover:bg-white/[0.06] transition-all"
          >
        ...
        {/* Card Size Slider */}
        <div className="flex items-center bg-surface-elevated/80 p-0.5 border border-white/[0.12] shadow-sm h-7 gap-1 px-2">
          ...
          <input
            ...
            className="w-16 h-1 accent-accent-cyan cursor-pointer"
            style={{ accentColor: 'var(--accent-cyan)' }}
          />
          <span className="text-[9px] font-mono text-slate-400 w-6 text-right">{cardScale}</span>
        </div>
